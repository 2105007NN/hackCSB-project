...
const journalController = async (req, res) => {
	try {
        ...
        const dbEntry = await db.all(`SELECT * FROM journals WHERE user_id = ${userId}`);
		...
		res.status(500).json(
			new ApiError(400, "ERROR IN Journal CONTROLLER")
		);
	}
};
...
const moodTrackerController = async (req, res) => {
	try {
        ...
        const dbEntry = await db.all(`SELECT * FROM mood_ratings WHERE user_id = ?`, [userId]);
